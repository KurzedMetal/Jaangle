...
!define APPNAMEANDVERSION "Jaangle 0.98i"
!define SETUPFILESPATH "H:\Bin\TeenSpirit\output\_SetupFiles\"

; Main Install settings
Name "${APPNAMEANDVERSION}"
InstallDir "$PROGRAMFILES\Jaangle"
InstallDirRegKey HKLM "Software\${APPNAME}" ""
OutFile "${SETUPFILESPATH}\jsetup.exe"
...
Section "-Main" MainSection
	...
	; Copy file to the program files directory
	SetOutPath "$INSTDIR\"
	File /a /r "${SETUPFILESPATH}\ProgramFiles\*.*"
	
	; Copy file to the common directory

	;SetOutPath "$APPDATA\Jaangle"
	;File /a /r "${SETUPFILESPATH}\Common\*.*"

	;AccessControl::GrantOnFile "$APPDATA\Jaangle" "(BU)" "GenericRead + GenericExecute + GenericWrite + Delete"
